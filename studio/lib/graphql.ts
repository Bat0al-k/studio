...
export async function executeGraphQL<T>({ query, variables, headers = {} }: ExecuteGraphQLOptions): Promise<GraphQLResponse<T>> {
  ...
  if (result.errors && typeof window !== 'undefined') {
    const isJwtExpired = result.errors.some((e: any) =>
      e.message?.includes('Could not verify JWT: JWTExpired') ||
    ...
    if (isJwtExpired) {
      ...
      // Return empty result to prevent immediate crashes before redirect
